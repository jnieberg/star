...
class Map {
	...
	renderPart(x, y, resolution, texture, scene, camera) {
	...
	renderFragment(resolution, iA, xA, yA, callback) {
		...
		wait(this.timerBank, () => {
			...
			if (i < 6) {
				...
				if (y < fragments) {
					if (x < fragments) {
						this.renderPart(x, y, resolution, this.textures[i], this.textureScenes[i], this.textureCameras[i]);
						this.renderFragment(resolution, i, ++x, y, callback);
					} else {
						this.renderFragment(resolution, i, 0, ++y, callback);
					}
				} else {
					this.renderFragment(resolution, ++i, 0, 0, callback);
				}
			} else if (callback) {
