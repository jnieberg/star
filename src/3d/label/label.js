...
export default function setLabel(text) {
	...
	if (text && text.length) {
		...
		const offset = textWidth + borderSize + 80;
		...
		TD.star.label = new THREE.Sprite(labelMaterial);
		TD.star.label.scale.set(0.0015 * (offset + width), 0.0015 * height);
		TD.star.label.material.sizeAttenuation = false;
		TD.star.label.material.depthTest = false;
		TD.scene.add(TD.star.label);
	}
}
